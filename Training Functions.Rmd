...
* remember that the placeholder variables in parameters and the function body are not defined and stored outside the function and hence, cannot be called as such
...
download_nonexist <- function(file_url, fetch = FALSE, path = ".") { # define that path must to be a string
  dest <- file.path(path, basename(file_url)) # basename is a neat function that strips away the path until and including the last path delimiter (e.g. /,\)
  if (fetch || !file.exists(dest))# file.exists returns a logical vector indicating whether the files named by its argument exist
    download.file(url = file_url, dest) # download.file is a base function used to download files from web
  print(dest)
}
```
## Read CSV into R
